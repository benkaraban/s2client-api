...
void DrawFeatureLayer8BPP(const sc2::ImageData& image_data, int off_x, int off_y) {
...
public:
    virtual void OnGameStart() final {
        sc2::renderer::Initialize("Feature layers", 50, 50, 4 * kDrawSize, 2 * kDrawSize);
    }

    virtual void OnStep() final {
        const SC2APIProtocol::Observation* observation = Observation()->GetRawObservation();

        const sc2::ImageData& placement_img = Observation()->GetGameInfo().placement_grid;
        //std::cout << "bpp:" << placement_img.bits_per_pixel << std::endl;
        DrawFeatureLayer8BPP(placement_img, 0, 0);

        const sc2::ImageData& path_img = Observation()->GetGameInfo().pathing_grid;
        //std::cout << "bpp:" << placement_img.bits_per_pixel << std::endl;
        DrawFeatureLayer8BPP(path_img, 2*kDrawSize, 0);
...
int main(int argc, char* argv[]) {
    ...
    coordinator.StartGame("Test/AlmostEmpty.SC2Map");
